...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/16 16:54:34 by adzahrao          #+#    #+#             */
/*   Updated: 2025/08/16 16:54:35 by adzahrao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						ft_pipe_one(t_pipe *pipe_data, t_cmd *arg,
								t_myenv *env);
void						ft_pipe_two(t_pipe *pipe_data, t_cmd **arg,
								t_myenv *env);
void						wait_pid(t_pipe *pipe_data, t_myenv **myenv);
void						init_pipe_data(t_cmd *arg, t_myenv *env,
								t_pipe *pipe_data);
int							create_pipe_and_fork(t_cmd *arg, t_myenv *env,
								t_pipe *pipe_data);
...
char						*process_token_content(char *line, int *i,
								char *token, t_token_state *st);
char						*handle_special_tokens(char *line, int *i,
								char *token);
char						*process_token_part(char *line, int *i, char *token,
								t_token_state *st);
...
int							extract_var_name(const char *token, int *i,
								char *var_name);
...
t_token						*handle_successful_expansion(t_token *tok,
								char *expanded, t_token *next, t_was was);
char						*expand_token(char *token, t_var *vars, char **env);

void						process_token_character(char *token,
								t_token_state *st, char *result,
								t_expand_context *ctx);
int							expand_variable(char *token, int *i, char *result,
								t_expand_context *ctx);
void						trim_token_values(t_token *tok, t_token *next,
								int was_quoted);
...
char						**env_copy_loop(t_myenv *temp, char **env,
								int count, int i);
char						*get_myenv_value(t_myenv *myenv, char *name);
int							is_redirection(t_token_type type);
void						free_env_array(char **env);

void						process_commands(t_token **tokens, t_var *vars,
								char **env, t_cmd **cmd);
t_cmd						*parse_commands(t_token **tokens, t_var *vars,
								char **env);
t_cmd						*parse_command(t_token **tokens, t_var *vars,
								char **env);
int							validate_command(t_cmd *cur, t_cmd **cmd);
int							handle_redirection(t_cmd *cmd, t_token **token_ptr,
								t_token **tokens, t_expand_context *ctx);
...
int							dispatch_redirection(t_cmd *cmd, t_token *next,
								t_token_type type, t_expand_context *ctx);
int							handle_redirections(t_cmd *cmd, t_token *next,
								t_token_type type);
...
int							create_and_link_token(t_token *token, char *str,
								int split_pos);

void						ft_cd(t_cmd **cmd, t_myenv **myenv,
								t_myenv_ex **myenv_ex);
void						ft_pwd(t_myenv **myenv);
void						ft_export(t_myenv_ex **myenv_ex, t_myenv **myenv,
								t_cmd **cmd);
void						ft_unset(t_myenv_ex **myenv_ex, t_myenv **myenv,
								t_cmd *str);
...
void						declare_env(t_myenv **myenv, t_myenv_ex **myenv_ex,
								char **env);

char						**my_get_path_split(t_myenv **myenv, char *path,
								char c);
char						*my_get_path(t_myenv *myenv, char *path);
char						*check_cmd(char **path, char *cmd);
int							check_builtin_cmd(t_cmd **cmd, t_myenv **myenv,
								t_myenv_ex **myenv_ex);
...
int							ft_strncmp_nv(const char *s1, const char *s2,
								size_t n);
...
int							validate_chars_before_equals(char *arg,
								int equals_index, t_myenv **myenv);
int							is_valid_first_char(char c);
int							find_equals_index(char *arg);
char						*check_val(char *str);
char						*create_cupy(char *dest);
void						process_export_arg(t_myenv_ex **myenv_ex,
								t_myenv **myenv, t_cmd *cmd, int i);
char						*create_cupy(char *dest);
char						*check_val(char *str);
int							find_equals_index(char *arg);
int							validate_chars_before_equals(char *arg,
...
void						set_variables(t_cmd **cmd, t_myenv **myenv,
								t_myenv_ex **myenv_ex, char **env);
...
int							process_heredoc_line(t_heredoc_params *params,
								t_expand_context *ctx);
void						write_heredoc_line(t_heredoc_params *params,
								t_expand_context *ctx, char *line);
void						process_quoted_char(char *line, t_token_state *st,
								char *result, t_expand_context *ctx);
void						handle_quoted_expansion(char *line,
								t_token_state *st, char *result,
								t_expand_context *ctx);
char						*expand_heredoc_line(char *line,
								t_expand_context *ctx);

int							process_variable_extraction(
								t_var_extraction_params *params,
								t_expand_context *ctx);
void						variable_lookup_and_assign(
								t_var_extraction_params *params,
								t_expand_context *ctx);
int							process_token_expansion(
								t_token_expansion_params *params,
								t_expand_context *ctx);
int							handle_expansion_result(
								t_token_expansion_params *params,
